...
DateRange = (function() {
  ...
  */
  ...
  DateRange.prototype.format = function(_directives) {
    var directives, fmt, isMatch, key, matchFmt, matchIf, _i, _len;
    directives = _directives || DateRange.defaultDirectives;
    if (!'__default__' in directives) {
    ...
    for (key in directives) {
      ...
      if (key === '__default__') {
        ...
        for (_i = 0, _len = key.length; _i < _len; _i++) {
          matchFmt = key[_i];
          isMatch = isMatch && ((this.start.format(matchFmt) === this.end.format(matchFmt)) === matchIf);
          matchIf = !matchIf;
        }
        if (isMatch) {
          return this._format(fmt);
  ...
  */
  ...
  DateRange.prototype.chunk = function(duration, boundary) {
    var begin, end, result;
    if (duration.slice(0, 4) === 'into') {
      boundary = moment.normalizeUnits(duration.slice(4));
      duration = moment.duration(1, boundary);
    }
    begin = moment(this.start);
